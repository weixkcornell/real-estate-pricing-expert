...
description: '空间计量与机器学习估值工艺：处理空间依赖与空间异质性，并构建可解释的大规模自动估值（AVM）。涵盖 SAR/SEM、GWR/MGWR、随机森林、XGBoost、SHAP 归因与不确定性量化。Triggers on "空间计量", "GWR", "地理加权", "AVM", "自动估值", "机器学习估值", "随机森林", "XGBoost"'
version: 0.1.0
user-invocable: true
